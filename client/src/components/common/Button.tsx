...
export interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'danger' | 'ghost' | 'solar';
  size?: 'sm' | 'md' | 'lg';
  isLoading?: boolean;
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variantStyles = {
    primary:
      'bg-amber-500 hover:bg-amber-400 text-slate-950 font-semibold focus:ring-amber-500 shadow-lg shadow-amber-500/20',
    solar:
      'bg-gradient-to-r from-amber-500 via-solar-400 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-bold focus:ring-amber-400 shadow-md shadow-amber-500/30',
    secondary:
      'bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 focus:ring-slate-600',
    danger:
      'bg-rose-600 hover:bg-rose-500 text-white focus:ring-rose-500 shadow-lg shadow-rose-600/20',
    ghost:
